...
public class MainActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener, ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        location_button.setOnTouchListener(new View.OnTouchListener() {
            @Override
            public boolean onTouch(View view, MotionEvent motionEvent) {
                if (motionEvent.getAction() == MotionEvent.ACTION_DOWN) {
        ...
        qr_button.setOnClickListener(new View.OnClickListener(){
            public void onClick(View v){
                if(loginIntent.getIntExtra("login", 0) == 1){
                    ...
                    intent.putExtra("id", loginIntent.getStringExtra("userID"));
                    intent.putExtra("location", getCurrentAddress(currentPosition));
                    intent.putExtra("longitude", Double.toString(currentPosition.longitude));
                    intent.putExtra("latitude", Double.toString(currentPosition.latitude));
                    startActivity(intent);
                }
                else{
                    Toast.makeText(getApplicationContext(), "로그인하라고", Toast.LENGTH_SHORT).show();
                }
            }
        });
...
//        }
    ...
    public void mOnPopupClick(View v){
        ...
        if(loginIntent.getIntExtra("login", 0) == 1){
            ...
            intent.putExtra("userId", loginIntent.getStringExtra("userID"));
    ...
    private void showResult(){
        ...
        // Marker icon 변경
        BitmapDrawable bitmapdraw1 = (BitmapDrawable)getResources().getDrawable(R.drawable.kickboard);
        Bitmap b1 = bitmapdraw1.getBitmap();
        Bitmap smallMarker1 = Bitmap.createScaledBitmap(b1, 85, 100, false);

        BitmapDrawable bitmapdraw2 = (BitmapDrawable)getResources().getDrawable(R.drawable.holder);
        Bitmap b2 = bitmapdraw2.getBitmap();
        Bitmap smallMarker2 = Bitmap.createScaledBitmap(b2, 85, 100, false);
        //
        ...
        try {
            ...
            while(iterator.hasNext()) {
                ...
                markerOptions.icon(BitmapDescriptorFactory.fromBitmap(smallMarker1)); // icon 적용
            ...
            public void run() {
                try {
                    ...
                    while (index < jsonArray.length()) {
                        PlaceInfo temp = gson.fromJson(jsonArray.get(index).toString(), PlaceInfo.class);
                        places.add(temp);
                        index++;
                    }
                } catch (Exception e) {
        ...
        while(iterator.hasNext()) {
            ...
            LatLng location = new LatLng(Double.parseDouble(placeInfo.getLongitude()),Double.parseDouble(placeInfo.getLatitude()));
            MarkerOptions markerOptions = new MarkerOptions();
            markerOptions.title("Place "+ placeInfo.getId());
            markerOptions.snippet("place "+ placeInfo.getId());//세부적인 내용
            markerOptions.icon(BitmapDescriptorFactory.fromBitmap(smallMarker2)); // icon 적용
            markerOptions.position(location);
            mMap.addMarker(markerOptions);
        }
    }

    // 서비스가 작동 중인지 확인하는 메소드
    public boolean isServiceRunningCheck() {
        ActivityManager manager = (ActivityManager) this.getSystemService(Activity.ACTIVITY_SERVICE);
        for (ActivityManager.RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
            if ("com.artifly.sharedplatform.RunningService".equals(service.service.getClassName())) {
                return true;
            }
        }
        return false;
